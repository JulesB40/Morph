...
@net.neoforged.fml.common.EventBusSubscriber(modid = "morph")
public final class StabilityGameTests {
    private static final net.minecraft.resources.Identifier ID = net.minecraft.resources.Identifier.fromNamespaceAndPath("morph", "all_vanilla_attributes");
    @net.neoforged.bus.api.SubscribeEvent
    public static void functions(net.neoforged.neoforge.registries.RegisterEvent event) {
        if (Boolean.getBoolean("morph.enableGameTests"))
            event.register(net.minecraft.core.registries.Registries.TEST_FUNCTION, helper -> helper.register(ID, StabilityGameTests::allVanillaAttributes));
    }
    @net.neoforged.bus.api.SubscribeEvent
    public static void tests(net.neoforged.neoforge.event.RegisterGameTestsEvent event) {
        if (!Boolean.getBoolean("morph.enableGameTests")) return;
        var environment = event.registerEnvironment(ID);
        net.minecraft.resources.ResourceKey<java.util.function.Consumer<GameTestHelper>> function = net.minecraft.resources.ResourceKey.create(net.minecraft.core.registries.Registries.TEST_FUNCTION, ID);
        event.registerTest(ID, new net.minecraft.gametest.framework.FunctionGameTestInstance(function,
            new net.minecraft.gametest.framework.TestData<>(environment, net.minecraft.resources.Identifier.withDefaultNamespace("empty"), 100, 0, true)));
    }

    public static void allVanillaAttributes(GameTestHelper helper) {
