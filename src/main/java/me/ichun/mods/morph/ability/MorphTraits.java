...
public final class MorphTraits {
    ...
    private static final Set<String> FIRE_IMMUNE = ids("blaze", "ender_dragon", "ghast", "magma_cube",
            "shulker", "strider", "vex", "wither", "wither_skeleton", "zoglin", "zombified_piglin");
    private static final Set<String> WATER_SENSITIVE = ids("blaze", "enderman", "snow_golem", "strider");
    private static final Set<String> SUN_SENSITIVE = ids("drowned", "phantom", "skeleton", "stray", "bogged", "zombie", "zombie_villager");
    private static final Set<String> WATER_ONLY = ids("cod", "salmon", "pufferfish", "tropical_fish",
            "squid", "glow_squid", "guardian", "elder_guardian");
    ...
    private static final Set<String> HIGH_STEP = ids("donkey", "drowned", "horse", "llama", "mule", "ravager", "skeleton_horse", "trader_llama", "zombie_horse");
    private static final Identifier STEP_MODIFIER = Identifier.fromNamespaceAndPath("morph", "trait_step_height");
    ...
    public static boolean rejectsEffect(Player player, MobEffectInstance effect) {
        ...
        return "minecraft:wither".equals(form)
                || isUndead(form) && (effect.is(MobEffects.POISON) || effect.is(MobEffects.REGENERATION))
                || "minecraft:wither_skeleton".equals(form) && effect.is(MobEffects.WITHER);
    }
    ...
    public static void afterDamage(LivingEntity target, DamageSource source) {
        ...
        if ("minecraft:cave_spider".equals(form)) target.addEffect(new MobEffectInstance(MobEffects.POISON, 140), attacker);
        else if ("minecraft:pufferfish".equals(form)) target.addEffect(new MobEffectInstance(MobEffects.POISON, 60), attacker);
        else if ("minecraft:husk".equals(form)) target.addEffect(new MobEffectInstance(MobEffects.HUNGER, 140), attacker);
    ...
    public static void tick(ServerPlayer player, String form) {
        ...
        if (step != null) {
            double amount = HIGH_STEP.contains(form) ? Math.max(0, 1 - step.getBaseValue()) : 0;
            if (amount == 0) step.removeModifier(STEP_MODIFIER);
            else if (step.getModifier(STEP_MODIFIER) == null || step.getModifier(STEP_MODIFIER).amount() != amount)
    ...
    public static double landMultiplier(String form) {
        if (form == null) return 1;
        return switch (form) {
            case "minecraft:cod", "minecraft:salmon", "minecraft:guardian", "minecraft:elder_guardian", "minecraft:dolphin", "minecraft:axolotl" -> 0.1;
